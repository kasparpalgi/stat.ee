...
The service [retrieves the cluster values](https://localhost:3000/step-by-step/yearly-cluster#class-definition) for the company from the data.
...
The service [retrieves the MEA values](/database/get-mea) based on the cluster name and subtracts the corresponding MEA value from each retrieved field.
...
The service [retrieves the SDS values](/database/get-sds) based on the cluster name and divides each field by the corresponding SDS value.
