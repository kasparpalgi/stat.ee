...
import {
  CompanyRepository as YearRepository,
 ...
 */
export default async function handleCompanyId(
  req: Request,
  res: Response
): Promise<any> {
  ...
  try {
    ...
    const monthlyCluster = await modelService.resolveMonthly(
      monthly,
      correlationID
    );
    const yearlyCluster = await modelService.resolveYearly(
      companyYear,
      correlationID
    );
    const prediction = await modelService.predictionResponse(
      year,
      yearlyCluster,
      monthlyCluster,
      correlationID
