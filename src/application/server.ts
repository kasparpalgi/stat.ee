...
const localUrl = ssl ? `https://localhost:${port}` : `http://localhost:${port}`;
...
export function runApp(app: any) {
    try {
        ...
        if (error.message == 'Exit the application') { 
            ...
            console.log('A SSL certificate is required to run the server in HTTPS mode.');
            console.log('You can generate a self-signed certificate by following the instructions in the README.md file.');
        } else {
            console.log('An unexpected error occurred while starting the server.');
            console.log(`Please check the error message for more details: ${error.message}`);
        }
        process.exit(1);
    }
}
